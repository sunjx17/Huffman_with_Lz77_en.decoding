...
void dec(char*from,char*to){
	struct _stat info;
	_stat(from, &info);

	int size = info.st_size;
	if(size<0){
		perror("file read null?");
		getchar();getchar();
		exit(0);
	}
	...
	fread(dest,1,size,f);
	fclose(f);
//	unsigned char head[256*4+4+4];
	unsigned int H=getuint(dest);
	unsigned int szs=getuint(dest+4);
	//printf("%x  %u\n",H,szs);
	unsigned int diclens[256];
	for(size_t i=0;i<256;i++){
		diclens[i]=getuint(dest+8+4*i);
		//printf("%u\n",diclens[i]);
	}
	unsigned char sdict[256];
	for(unsigned int i=0;i<256;i++){
		sdict[i]=i;
	}
	unsigned char *outdic=new unsigned char[40960];
	size_t now_out=0;
	...
	for(size_t i=0;i<szs;i++){
		tmp=getbit(dest+256*4+4+4,i);
		if(tmp)now=((Huff_pars*)now)->rch;
		else now=((Huff_pars*)now)->lch;
		if(now->IsLeave){
			if(now_out>=40960){
				fwrite(outdic,1,40960,f);
				now_out=0;
			}

...
***************************************************************************/
int bitof(int n){
	return (int)(ceil(log((double)n)/log((double)2)));
}
...
struct bitFILE* bitIO_open(const char *path, int mode){
	...
	/* errors handler */
	if(mode!=BIT_IO_W &&mode!=BIT_IO_WB && mode!=BIT_IO_R)
	{
	...
	/* write mode */
	else if(bitF->mode == BIT_IO_WB)
	{
		if((bitF->file = fopen(path, "wb")) == NULL)
		return NULL;
	}
	else if(bitF->mode == BIT_IO_W)
	{
		if((bitF->file = fopen(path, "wb")) == NULL)
		return NULL;
	}
...
int bitIO_close(struct bitFILE *bitF){
	...
	if(bitF->mode == BIT_IO_W ||bitF->mode == BIT_IO_WB )
	{
		if(bitF->bitpos > 0)
			(bitF->bytepos)++;
		write_buffer(bitF);
...
int bitIO_write(struct bitFILE *bitF, void *info, int nbit){
	...
	if(bitF == NULL || bitF->file == NULL || (bitF->mode != BIT_IO_W &&bitF->mode != BIT_IO_WB) || info == NULL || nbit < 0)
		return -1;
...
size_t bitio_getnowpos(bitFILE*f){
	return (f->bytepos)*8+f->bitpos;
}
int bitIO_w_putchar(bitFILE *bitF,size_t bytepos,void* value){
	if(bitF == NULL || bitF->file == NULL || (bitF->mode != BIT_IO_W &&bitF->mode != BIT_IO_WB) )return -1;
	if(bitF->haswrittened){
		fseek(bitF->file,bytepos,SEEK_SET);
		fputc(*(unsigned char*)value,bitF->file);
		fseek(bitF->file,0,SEEK_END);
	}
	else{
		bitF->buffer[bytepos]=*(unsigned char*)value;
	}
	return 0;
}
...
//	int byte_pos = 0, bit_pos = 0; /* byte's position and bit's position */
	...
	/* if it is a 1 set it, otherwise do nothing */
	bool ret=(bitF->buffer[bitF->bytepos] & mask);
